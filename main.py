...
class BotonLindo(Button):
    def __init__(self, text, color_fondo, **kwargs):  # Cambiado 'texto' por 'text'
        super().__init__(**kwargs)
        self.text = text
...
class Calculadora(BoxLayout):
    def __init__(self, **kwargs):
        ...
        for texto, color, tipo, valor_logico in botones:
            btn = BotonLindo(text=texto, color_fondo=color)
            
            # Corrección crítica aquí: v=valor_logico guarda el estado actual en cada iteración
            if tipo == 'num':
